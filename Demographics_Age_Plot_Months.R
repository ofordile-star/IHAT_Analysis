...
cat(sprintf("• Total samples: %d (%d unique subjects)\n", 
            nrow(df_demo), n_distinct(df_demo$`Randomisation No`)))

cat(sprintf("• Age range: %.1f–%.1f months (mean ± SD: %.1f ± %.1f months)\n",
...
cat(sprintf("• Age difference test p = %.3f\n", age_test$p.value))
cat(sprintf("• Age group distribution test p = %.3f\n", chisq_age_group$p.value))
cat(sprintf("• Gender distribution test p = %.3f\n", chisq_gender$p.value))

# ================================
# VISUALIZATION PLOTS
# ================================

# PLOT 1: Age Distribution by Illness Status
...
# PLOT 2: Prevotella stercorea Abundance by Age Group
df_d <- df %>%
...
# PLOT 3: Illness Distribution by Age Group
df_e <- df %>%
  filter(!is.na(Ill_Status), !is.na(age_group))
...
cat("\n\nAnalysis complete. Outputs saved to Desktop.\n")
