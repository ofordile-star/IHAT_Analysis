...
df_corr <- df %>%
  filter(!is.na(`Prevotella_stercorea_7.05`), !is.na(Ill_Status), !is.na(age_group)) %>%
  mutate(
    logP = log(`Prevotella_stercorea_7.05` + 1),
...
# Write full glm summary to CSV (numeric columns rounded to 4 dp as before)
write_csv(glm_summary %>%
            mutate(across(where(is.numeric), ~round(., 4))),
          "C:/Users/oofordile/Desktop/AgeStratified_LogisticRegression_4dp.csv")

# Extract interaction terms and save (rounded to 4 dp)
...
# Interaction p-value label to 3 decimal places (safely handle if no interaction terms)
if (nrow(interaction_terms) > 0) {
  # Use the maximum p-value among interaction terms (as in your original)
  interaction_p_val <- max(interaction_terms$p.value, na.rm = TRUE)
  interaction_p_label <- paste0("Interaction P = ", formatC(interaction_p_val, format = "f", digits = 3))
} else {
...
plot_a_inner <- ggplot(df_corr, aes(x = Ill_Status, y = logP, fill = age_group)) +
  ...
  theme_minimal(base_size = 10) +  # Reduced by 2 units
  theme(
...
# Compose Panel A with title + label
plot_a <- ggdraw() +
...
plot_b_inner <- ggplot(ill_df, aes(x = age_group, y = count, fill = age_group)) +
  ...
  theme_minimal(base_size = 10) +  # Match Panel A
  theme(
    legend.position = "none",
    panel.grid = element_blank(),
    strip.text = element_text(size = 12),
...
# Combine Plots
# ======================================================
combined_plots <- cowplot::plot_grid(plot_a, plot_b, ncol = 2, rel_widths = c(1, 1))

# ======================================================
# Export Plots (PDF + EMF)
# ======================================================
pdf("C:/Users/oofordile/Desktop/Figure_Combined_AgeGroup_Analysis.pdf", width = 14, height = 6)
print(combined_plots)
dev.off()

emf("C:/Users/oofordile/Desktop/Figure_Combined_AgeGroup_Analysis.emf", width = 14, height = 6)
print(combined_plots)
dev.off()
