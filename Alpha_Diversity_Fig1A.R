...
# ======================================================
# UPDATED: Mixed-model / GLM comparison function
# ======================================================
run_age_stratified_model <- function(data, metric, timepoint, age_group) {
  ...
  return(list(p_value = p_val, n = nrow(age_data), estimate = estimate,
              ci_lower = ci_lower, ci_upper = ci_upper,
              F = F_val, df1 = df1, df2 = df2, partial_eta2 = partial_eta2))
}

# ======================================================
# Helper function for age-adjusted model
# ======================================================
extract_lm_term_stats <- function(fit, term_name) {
  an_tbl <- tryCatch(as.data.frame(anova(fit)), error = function(e) NULL)
  res <- list(term = term_name, F = NA, df1 = NA, df2 = NA, p_value = NA,
  ...
  if(!is.null(an_tbl) && term_name %in% rownames(an_tbl)) {
    ...
    partial_eta2 <- ss_term / (ss_term + ss_resid)
    res$F <- F_val; res$df1 <- df_term; res$df2 <- df_resid
    res$p_value <- p_val; res$partial_eta2 <- partial_eta2
  }
  coef_try <- tryCatch({
    cf <- coef(fit)
...
for(tp in c("D1", "D15", "D85")) {
  ...
  for(m in all_metrics) {
    ...
    row <- data.frame(Timepoint = tp, Metric = m, Analysis = "Age-adjusted", Term = "Ill_Status",
                      Estimate = term_stats$estimate, CI_lower = term_stats$ci_lower,
                      CI_upper = term_stats$ci_upper, F = term_stats$F, df1 = term_stats$df1,
                      df2 = term_stats$df2, p_value = term_stats$p_value,
                      partial_eta2 = term_stats$partial_eta2, n = term_stats$n)
  ...
  for(tp in c("D1", "D15", "D85")) {
    for(m in all_metrics) {
      ...
      if(!is.na(stats$p_value)) {
        row <- data.frame(AgeGroup = age_grp, Timepoint = tp, Metric = m,
                          Analysis = "Age-stratified", Term = "Ill_Status",
                          Estimate = stats$estimate, CI_lower = stats$ci_lower,
                          CI_upper = stats$ci_upper, F = stats$F, df1 = stats$df1,
                          df2 = stats$df2, p_value = stats$p_value,
                          partial_eta2 = stats$partial_eta2, n = stats$n)
...
# ======================================================
# Combine and create Nature-style stats column
# ======================================================
...
combined_results$Nature_Stats <- sapply(1:nrow(combined_results), function(i) {
  ...
  context <- if(!is.na(row$AgeGroup)) paste0(row$AgeGroup, ", Age-stratified") else "Age-adjusted"
  sprintf("%s (%s, %s): F(%g, %g) = %.2f, p = %.3f, p_adj = %.3f, eta2_p = %.3f, 95%% CI [%.2f, %.2f]; n = %d",
          row$Metric, row$Timepoint, context, row$df1, row$df2, row$F,
          row$p_value, row$p_adj, row$partial_eta2, row$CI_lower, row$CI_upper, row$n)
})
...
all_timepoint_results_adjusted <- combined_results %>%
  ...
  dplyr::select(Timepoint, Metric, p_value, p_adj, n, Estimate, CI_lower, CI_upper,
                F, df1, df2, partial_eta2)
all_timepoint_results_by_age <- combined_results %>%
  filter(Analysis == "Age-stratified") %>%
  dplyr::select(AgeGroup, Timepoint, Metric, p_value, p_adj, n, Estimate,
...
create_condensed_plot <- function(data, metric, y_label, colors, results_df,
  ...
  if(!is.null(age_grp)) {
    ...
    } else stats_df <- results_df %>% filter(Metric == metric)
  }
  ...
  p <- ggplot(data, aes(x = timepoints, y = .data[[metric]], color = Ill_Status,
    ...
    geom_point(position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.6),
               alpha = 0.4, size = 1.8) +
    geom_boxplot(aes(fill = Ill_Status), alpha = 0.3, outlier.shape = NA,
