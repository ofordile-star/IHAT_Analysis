...
Nature Standard Figure Template Code
# --- Nature-Standard Figure Template ---

# Load libraries
library(ggplot2)
library(Cairo)   # safer PDF export

# Example dataset
df <- data.frame(
  group = rep(c("Not-Ill", "Ill"), each = 10),
  value = c(rnorm(10, 5, 1), rnorm(10, 7, 1))
)

# Define Nature-style theme
nature_theme <- theme_bw(base_size = 14, base_family = "Helvetica") + 
  theme(
...
# Define consistent Nature color scheme (example)
nature_colors <- c("Not-Ill" = "#D73027", "Ill" = "#4575B4")  # red vs blue

# Create plot
p <- ggplot(df, aes(x = group, y = value, fill = group)) +
  geom_boxplot(width = 0.5, colour = "black", size = 0.8) +
  scale_fill_manual(values = nature_colors) +
  labs(x = "Health status", y = "Relative abundance (%)") +
  nature_theme

# Export as vector PDF (no distortion, perfect zoom)
CairoPDF("Figure1.pdf", width = 6, height = 6)  # adjust size as needed
print(p)
dev.off()
