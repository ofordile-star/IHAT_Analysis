...
alpha_data$Ill_Status <- factor(alpha_data$Ill_Status, levels = c("Not-Ill", "Ill"))
...
run_comparison_detailed <- function(data, metric, group_var) {
  ...
  if(same_timepoint) {
    ...
    # Mixed model for different timepoints
    ...
    result <- tryCatch({
      ...
      if(comp_name %in% rownames(coef_summary)){
        ...
        ci <- confint(fit, parm=comp_name, method="Wald")
        ci_lower <- ci[1]; ci_upper <- ci[2]
      } else { estimate <- ci_lower <- ci_upper <- NA_real_ }
    ...
    p_val <- result$p_val; f_stat <- result$f_stat; df1 <- result$df1; df2 <- result$df2
    estimate <- result$estimate; ci_lower <- result$ci_lower; ci_upper <- result$ci_upper
    partial_eta2 <- result$partial_eta2
  }
  
  return(data.frame(n=n, Estimate=estimate, CI_lower=ci_lower, CI_upper=ci_upper,
                    F=f_stat, df1=df1, df2=df2, partial_eta2=partial_eta2, p_value=p_val))
}

# ======================================================
# Run all comparisons with stats
# ======================================================
...
# ======================================================
# Nature-style reporting
# ======================================================
all_detailed_results <- all_detailed_results %>%
  mutate(Nature_Report = paste0(
    Metric, " (Age-adjusted): ",
    "F(", round(df1,2), ", ", round(df2,2), ") = ", round(F,2),
    ", p = ", signif(p_adj,3),
...
fdr_summary <- all_detailed_results %>%
  ...
  rename(
    ...
    `p adj Simpson` = Simpson,
    `p adj Pielou` = Pielou,
    `p adj Fisher` = Fisher
  )

write.csv(fdr_summary,
          "C:/Users/oofordile/Desktop/Alpha_Diversity_FDR_Summary.csv",
          row.names = FALSE)

# ======================================================
# Plotting function
# ======================================================
create_comparison_plots_nature <- function(alpha_data, all_detailed_results, metrics, comparisons, group_colors) {
  plot_list <- list()
...
# Generate plots
# ======================================================
comparison_plots <- create_comparison_plots_nature(alpha_data, all_detailed_results, metrics, comparisons, group_colors)

# ======================================================
# Create Canvas PDFs + EMFs
# ======================================================
